...
def _enumerate_tableaux_with__recursive(current_tableau, current_value, weights, max_division):
    ...
    for new_division in _expand_division_iter(current_tableau.shape.division, weight, max_division):
        new_numbers = [[v for v in line] for line in current_tableau.numbers]
        new_numbers.append([])
...
class YoungDiagram(FiniteSequence):
    ...
    def enumerate_standard_tableaux(self):
        ...
        for tableau in self.enumerate_tableaux_with([1]*self.n):
            yield StandardYoungTableaux(tableau)
...
class NumberedYoungDiagram(YoungDiagram):
    ...
    def add_box(self, i, number):
        ...
        ヤング図形に箱を追加する
        '''
        new_numbers = [[v for v in line] for line in self.numbers]
    ...
    def enumerate(self):
        ...
        for i in range(len(self.numbers)):
            yield i, ((j, self.numbers[i][j]) for j in range(len(self.numbers[i])))
...
class YoungTableaux(NumberedYoungDiagram):
    ...
    def reverse_row_bump(self, index):
        ...
        for i in range(index-1, -1, -1):
            line, value = _reverse_bump_line(new_numbers[i], value)
            new_numbers[i] = line
    ...
    def enumerate_boxes_numbering_order(self, reverse=False):
        '''
        箱を番号順に列挙する
        '''
        number_box_list = []
        for i in range(len(self.numbers)):
            for j in range(len(self.numbers[i])):
                number_box_list.append(((self.numbers[i][j], j), (i, j)))
        
        number_box_list.sort(reverse=reverse)

        for _, box in number_box_list:
            yield box # 箱を返す


class StandardYoungTableaux(YoungTableaux):
    def __init__(self, numbers : list[list[int]]):
        super().__init__(numbers)
        if not self.is_standard():
            raise ValueError("Invalid Standard Young tableaux")
        

    def add_next_box(self, i):
        '''
        次の箱を追加する
        '''
        new_numbers = [[v for v in line] for line in self.numbers]
        if i == len(new_numbers):
            new_numbers.append([])
        new_numbers[i].append(self.n + 1)
        return self.__class__(new_numbers)
    

    @classmethod
    def from_box_order(cls, index_list: list[tuple[int, int]] | list[int]):
        '''
        箱の順番からStandardYoungTableauxを生成する

        index_list: list[tuple[int, int]] | list[int] 箱の(i, j)の順番、またはiの順番
        '''
        if not index_list:
            return cls([])
        
        if isinstance(index_list[0], tuple):
            index_list = [box[0] for box in sorted(index_list)]

        result = cls([])
        for i in index_list:
            result = result.add_next_box(i)

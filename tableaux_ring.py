...
class YoungTableauxRing:
    ...
    def _erase_zeros(self):
        ...
        簡約化
        '''
        for tableau, value in self.coef_dict.copy().items():
            if value == 0:
                del self.coef_dict[tableau]

    def substitute(self, values: list[int | float | typing.Any]):
        '''
        対応する対称多項式に値を代入する
        '''
        result = 0
        for tableau, coef in self.coef_dict.items():
            monomial = 1
            for index in tableau.to_word():
                if index > len(values):
                    monomial = 0
                    break
                else:
                    monomial *= values[index-1]
            result += coef * monomial
...
def _vandermonde_determinant(values: list[int | float]):
    '''
    Vandermonde行列式を求める
    '''
    n = len(values)
...
class SymmetricYoungTableauxRing:
    ...
    def substitute(self, values: list[int | float | typing.Any]):
        '''
        TODO: 対応する対称多項式に値を代入する。ヤコビ・トゥルーディ公式を用いる。因数分解形式の多項式に変換する必要がある(同じ値を含む場合があるため)
        '''
        return self.expand(len(values)).substitute(values) # 一時的な処置
    
        # if self.max_value is not None:
        #     if self.max_value < len(values):
        #         raise ValueError('values is too long')
        #     values = values + [0] * (self.max_value - len(values))
        #     max_value = self.max_value
        # else:
        ...
        # for diagram, coef in self.coef_dict.items():
        #     matrix = np.array([
        #         [values[j]**(max_value + diagram.division[i] - i - 1) for j in range(len(values))] 
        #         for i in range(len(diagram.division))
        #     ])
        #     numerator += coef * np.linalg.det(matrix)
        # denominator = _vandermonde_determinant(values)
